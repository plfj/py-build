...
_check_tools() {
    ...
    if [[ "$missing" -ne 0 ]]; then
        _die "${missing} required tool(s) missing."
    fi
...
_XZ_VERSION="5.6.3"
_SQLITE_VERSION="3490100"   # 3.49.1
_SQLITE_YEAR="2025"
...
_build_deps() {
    ...
    _dl_extract() {
        ...
        if [[ -d "$dir" ]]; then _info "  Already extracted: $(basename "$dir")"; return 0; fi
        mkdir -p "$(dirname "$dir")"
        case "$tarball" in
    ...
    if [[ ! -f "${CROSS_DEPS_PREFIX}/lib/libbz2.a" ]]; then
        ...
        _dl_extract "https://sourceware.org/pub/bzip2/bzip2-${_BZIP2_VERSION}.tar.gz" \
            "bzip2-${_BZIP2_VERSION}.tar.gz" "$bz2_src"
        make -C "$bz2_src" -j"${TERMUX_PKG_MAKE_PROCESSES}" \
            CC="${_cc}" AR="${_ar}" RANLIB="${_ranlib}" CFLAGS="${_cflags} -fPIC" \
    ...
    if [[ ! -f "${CROSS_DEPS_PREFIX}/lib/liblzma.a" ]]; then
        ...
        _dl_extract \
            ...
            "xz-${_XZ_VERSION}.tar.xz" "$xz_src"
        ( mkdir -p "${xz_src}/cross-build" && cd "${xz_src}/cross-build"
          "${xz_src}/configure" --prefix="${CROSS_DEPS_PREFIX}" \
              ...
              > "${log_dir}/xz.log" 2>&1 \
              || { cat "${log_dir}/xz.log"; _die "xz configure failed."; }
          make -j"${TERMUX_PKG_MAKE_PROCESSES}" >> "${log_dir}/xz.log" 2>&1 \
              || { cat "${log_dir}/xz.log"; _die "xz build failed."; }
    ...
    # ── sqlite3 ───────────────────────────────────────────────────────────────
    # NOTE: --enable-json1 was removed. JSON1 is built-in by default since
    # SQLite 3.38.0 (2022-02-22) and the --enable-json1 configure option was
    # removed in SQLite 3.47.0. Passing it to 3.49.x causes an immediate
    # "Unknown option --json1" error.
    ...
    if [[ ! -f "${CROSS_DEPS_PREFIX}/lib/libsqlite3.a" ]]; then
        ...
        _dl_extract \
            ...
            "sqlite-autoconf-${_SQLITE_VERSION}.tar.gz" "$sq_src"
        ( mkdir -p "${sq_src}/cross-build" && cd "${sq_src}/cross-build"
          "${sq_src}/configure" --prefix="${CROSS_DEPS_PREFIX}" \
              --host="${TERMUX_HOST_PLATFORM}" --build="${TERMUX_BUILD_TUPLE}" \
              --disable-shared --enable-static \
              --enable-fts5 \
              CC="${_cc}" CFLAGS="${_cflags} -fPIC" LDFLAGS="${_ldflags}" \
    ...
    if [[ ! -f "${CROSS_DEPS_PREFIX}/lib/libssl.a" ]]; then
        ...
        _dl_extract \
            ...
            "openssl-${_OPENSSL_VERSION}.tar.gz" "$ssl_src"
        (
            cd "$ssl_src"
            local ossl_target
            case "${TERMUX_ARCH}" in
    ...
    if [[ ! -f "${CROSS_DEPS_PREFIX}/lib/libffi.so"  ]] || \
       [[ ! -f "${CROSS_DEPS_PREFIX}/include/ffi.h"  ]]; then
        ...
        _download "${_LIBFFI_TERMUX_BASE_URL}/libffi_${_LIBFFI_VERSION}_${_ffi_arch}.deb" "$ffi_deb"
        local ffi_extract="${deps_build}/libffi-deb"
        rm -rf "$ffi_extract"; mkdir -p "$ffi_extract"
        local ffi_tmp="${ffi_extract}/tmp"; mkdir -p "$ffi_tmp"
        ( cd "$ffi_tmp"; ar x "$ffi_deb"
    ...
    if [[ ! -f "${CROSS_DEPS_PREFIX}/lib/libncursesw.so"        ]] || \
       ...
       [[ ! -f "${CROSS_DEPS_PREFIX}/include/ncursesw/curses.h" ]]; then
        ...
        for deb in "$nc_lib_deb" "$nc_static_deb"; do
            local deb_tmp="${nc_extract}/tmp_$(basename "$deb")"; mkdir -p "$deb_tmp"
            ( cd "$deb_tmp"; ar x "$deb"
              local data_tar; data_tar="$(ls data.tar.* 2>/dev/null | head -1)"
...
_verify_modules() {
    ...
    if [[ ! -x "$python_bin" ]]; then
        _warn "python${_MAJOR_VERSION} not executable at $python_bin — skipping."
        return
    fi
    local failed=0
